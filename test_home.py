...
def test_02_home_2(page, goods_num, case_id, request):
    request.node._testrail_case_id = case_id
    ...
    with contextlib.redirect_stdout(output_content):
        ...
        # BT 모듈로 이동 후 확인
        parent = home_page.home_module_by_title("함께 구매하면 좋은 상품이에요")
        # 광고 태그 확인
        result = home_page.check_bt_ad_tag(parent)
        goodscode = result["goodscode"]
        # 광고상품 상품 번호 추출
...
# def test_01_home_1(page, keyword, case_id, request):
...
#         home_page.search_module_by_title("이 상품을 본 고객들이 함께 찾는 상품")
        #print("완료")
        # 먼저 둘러보세요 모듈내 광고 상품 곽인
       #goodscode = home_page.assert_item_in_module("이 상품을 본 고객들이 함께 찾는 상품")
        # 상품 번호 저장
...
# vimp_db_ai = None
#
# def test_05_fetch_from_db(file_start_time, file_start_dt, file_start_hour):
...
# def test_06_srp_3(keyword, case_id, request):
#     # TestRail 케이스 ID를 현재 실행 노드에 저장
#     request.node._testrail_case_id = case_id
...
# def test_07_srp_4(keyword, case_id, request):
#     # TestRail 케이스 ID를 현재 실행 노드에 저장
#     request.node._testrail_case_id = case_id
#     db_check = DatabricksSPClient()
#     with open("json/test_srp.json", "r", encoding="utf-8") as f:
