...
import pytest
from case_data.home_data import home_testcases1, home_testcases2
import json
import io
import contextlib
...
def test_01_home_1(page, goods_num, case_id, request):
    ...
    with contextlib.redirect_stdout(output_content):
        ...
        # VIP 상품 번호와 RVI_VT_CPC 모듈 타이틀 좌측 상품 번호 같은지 확인
        ...
        # 상품 노출 확인시간 저장
        logger.record_time("case1", goodscode, "exposure")
        # 상품 번호 저장
        logger.record_goodscode("case1",goods_num, goodscode)
        target = result["target"]
        # 상품 클릭시간 저장
        logger.record_time("case1", goodscode, "click")
        # 상품 클릭후 해당 vip 이동 확인
...
def test_03_fetch_from_db(file_start_time, file_start_dt, file_start_hour):
    ...
    with open("json/test_home.json", "r", encoding="utf-8") as f:
        data = json.load(f)
...
@pytest.mark.flaky(reruns=2, reruns_delay=1)
@pytest.mark.parametrize("goods_num, case_id", home_testcases2,ids=[c for _, c in home_testcases2])
def test_04_home_2(goods_num, case_id, request):
    # TestRail 케이스 ID를 현재 실행 노드에 저장
    request.node._testrail_case_id = case_id
